...
output: 
  html_document: 
    ...
    toc_float: true
knit: (function(input, ...) {
  rmarkdown::render(input, output_file = paste0("2026 Q4", " DH Quarterly Reporting", " Model 1 Incidents"), output_dir = "../Reports") })
---
...
nephu_incident_outbreaks <- outbreaks_nephu %>%
  ...
  #
  # Q4 2026 only - exclude Kinn Thai Hvittingfoss - part of statewide Model 3 outbreak
  dplyr::filter(event_id != "320266259290") %>% 
  #
...
nephu_highrisk <- completed_nephu %>% 
  ...
  dplyr::filter(work_status %in% c("Employed", "Child in child/day care") | !is.na(occupation))
